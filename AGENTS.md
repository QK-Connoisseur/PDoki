...
Pumdoki is an adult creator platform (subscriptions, paid content, creator services/bookings, real-time messaging, prepaid "Veso" credits, and a collectible "Oasis/Drimy" retention game). The repo is an npm-workspaces monorepo: a **React frontend prototype** in `apps/web`, an independently buildable private operations shell in `apps/admin`, a TypeScript Express API in `apps/api`, shared Zod schemas in `packages/contracts`, and Prisma + PostgreSQL in `packages/database`. Phase 3 is published and CI-verified through commit `d55f5f3`. Phase 4 Slice 1 is published and CI-verified as commit `ce6c9e4`: verified members can submit one persisted creator application, versioned prototype acceptances are recorded atomically, and the result remains pending without role promotion or Dashboard access. Phase 4 Slice 2 is published as a fail-closed non-approval review/evidence foundation. PR #15 published Slice 3 as `24e1653`, adding dormant provider-neutral assertion verification, database-owned exact operator/permission authorization, and request-integrity seams; post-merge run `32784338614` passed all jobs. The founder approved the seven private-operations policy decisions, including the two-lock identity-plus-Pumdoki-authorization model. The founder authorized Slice 4 publication on 2026-08-30; implementation `317abda` is pushed in [draft PR #17](https://github.com/QK-Connoisseur/PDoki/pull/17) against `dev`. Implementation-head CI run `33337349347` passed on that commit; the PR is not merged. Every later reconciliation head requires fresh CI before review or merge. Its unmounted Cloudflare candidate and credential-redaction coverage passed `66/66` focused local checks with synthetic keys only. On 2026-08-25, the founder independently authenticated both existing Cloudflare account hardware-key enrollments on the new Mac; this is limited physical/browser evidence, not a real Access assertion or signed hardware-method proof. Cloudflare is not selected. The public API still does not mount the router; deployment, live configuration, provider selection, and activation remain separately gated, and G1–G12 remain `NOT EVALUATED`. Identity collection, counsel-approved policies, production-ready operational authentication/review, payments, media, and other product APIs remain incomplete. `packages/ui` is still empty scaffolding.
...
- `apps/api/src/operations` — dormant, provider-neutral private-operations trust primitives: strict signed-assertion validation, database-owned exact operator/permission resolution, and test-only request-integrity composition. Slice 4 implementation `317abda`, pushed in draft PR #17, adds only an unmounted candidate Cloudflare assertion verifier with synthetic-key tests. Implementation-head CI run `33337349347` passed on `317abda`. Every later reconciliation head requires fresh CI before review or merge. Publication closes out completed local work; further private-operations development remains parked. Both existing account hardware-key enrollments passed founder-attested Mac browser authentication, but no exact Access application assertion, signed hardware-method claim, or AAGUID evidence was reviewed; Cloudflare is not selected. The normal API/server do not import or mount the candidate or creator-review router; no production provider adapter, first-party operations session, private origin, runtime database role, or deployment exists.
...
The monorepo migration is committed: work against `apps/web` and `apps/api`, never old top-level paths. Phase 2 is **partially complete (published fixed-canary foundation)** — PR #13 merged the application-owned PostgreSQL worker and non-secret canary into `dev` as `6311522`, with exact-head and post-merge CI green. Staging deploy, RDS, backups, Sentry, Redis/shared throttling, general idempotency, and product-flow migration remain deferred. Phase 3 is published and CI-verified through `d55f5f3`. Phase 4 Slices 1–2 are published; PR #15 published dormant Slice 3 as `24e1653`, with post-merge run `32784338614` green. The founder-approved decisions are recorded in `docs/architecture/phase4-private-operations-founder-policy-decisions.md`. The founder authorized Slice 4 publication on 2026-08-30. Implementation `317abda` on `codex/phase4-yubikey-claim-schema-evaluation` is pushed in draft PR #17 against `dev`; implementation-head CI run `33337349347` passed. Every later reconciliation head requires fresh CI before review or merge, and readiness or merge requires separate approval. Its unmounted Cloudflare candidate and logger coverage passed `66/66` focused local checks with synthetic keys only. This publication is close-out only: further private-operations development remains parked under the newer local plan. The next proposed engineering priority is bounded content-domain design, not live Access testing. Founder-attested account-level Mac tests passed with both existing hardware-key enrollments, while exact Access assertion/AMR evidence remains absent. The router remains unmounted; G1–G12, provider/live configuration, runtime roles, deployment, and activation remain separately gated. The founder approved the PLAN-aligned tracker, and its 2026-08-25 reconciliation preserves all legacy records and contains no restricted key evidence. Notifications, theme, billing, export/deletion, identity-provider integration, production-ready operational creator review, and server-side content-query enforcement remain deliberately sequenced to their dependency phases. See `HANDOFF.md` for exact commands, counts, and publication status.
