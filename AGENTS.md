...
- `apps/api/src/durableJobs` + `apps/api/src/worker` — feature-branch Phase 2 durable-worker foundation. A fixed non-secret canary is enqueued atomically through Prisma, then processed by a separate PostgreSQL worker with opaque lease fencing, bounded retries, terminal states, redacted telemetry, and bounded shutdown. No current product flow uses it, and no pull request or merge is authorized yet.
