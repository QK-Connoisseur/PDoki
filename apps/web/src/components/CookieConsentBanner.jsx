      ...
      {!showPreferences ? (
        <div className="mx-auto flex max-w-7xl flex-wrap items-center gap-4 px-5 py-4 sm:flex-nowrap sm:gap-6">
          <div className="flex shrink-0 items-center gap-2.5">
            ...
            <p className="text-sm font-semibold text-[#241a22]">We use cookies</p>
          </div>
          <p className="min-w-0 flex-1 text-xs leading-relaxed text-[#8c6d7f]">
            Pumdoki uses cookies and similar technologies to provide, improve, and personalise our services. By clicking "Accept All" you consent to our use of cookies.{" "}
            <button
        ...
        <div className="mx-auto max-w-lg px-5 py-5">
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-sm font-bold text-[#241a22]">Manage Cookie Preferences</h2>
            <button
          ...
          <div className="space-y-3">
            {[
              ...
              { key: "functional", label: "Functional", desc: "Enables features like personalised settings and saved preferences." },
              { key: "analytics", label: "Analytics", desc: "Helps us understand how visitors interact with our platform." },
              { key: "marketing", label: "Marketing", desc: "Used to deliver relevant ads and measure campaign effectiveness." },
            ].map(({ key, label, desc, locked }) => (
              <div key={key} className="flex items-start justify-between gap-3 rounded-xl border border-pink-50 bg-pink-50/40 px-3 py-2.5">
                <div>
                  <p className="text-xs font-semibold text-[#241a22]">{label}</p>
                ...
                >
                  <span
                    ...
                    style={{ left: "3px", transform: (locked || prefs[key]) ? "translateX(16px)" : "translateX(0)" }}
                  />
                </button>
              </div>
            ))}
          ...
          <div className="mt-4 flex gap-2">
            ...
            <button
              ...
              style={{ background: "linear-gradient(90deg,#FF4D8D,#f472b6)", boxShadow: "0 2px 12px rgba(255,77,141,0.28)" }}
            >
              Save Preferences
            </button>
          </div>
