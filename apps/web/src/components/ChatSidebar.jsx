...
const MOCK_MESSAGES = {
  1: [
    { id: 1, text: "Hey! Just wanted to say your content is amazing 💕", fromMe: false, time: "2:10 PM" },
    { id: 2, text: "That genuinely made my day, thank you so much! 🌸", fromMe: true, time: "2:12 PM" },
    { id: 3, text: "When's your next drop? I'm so excited!", fromMe: false, time: "2:14 PM" },
    { id: 4, text: "This weekend! Something really special 🎀 Stay tuned", fromMe: true, time: "2:16 PM" },
    { id: 5, text: "Thanks for the support!", fromMe: false, time: "2:18 PM" },
  ],
  2: [
    { id: 1, text: "Are you streaming tonight?", fromMe: true, time: "6:40 PM" },
    { id: 2, text: "Yes! 9pm as always 🎵 Don't miss it", fromMe: false, time: "6:42 PM" },
    { id: 3, text: "I'll be there 🙌", fromMe: true, time: "6:44 PM" },
    { id: 4, text: "See you on the stream tonight!", fromMe: false, time: "6:50 PM" },
  ],
  3: [
    { id: 1, text: "Your photography is absolutely stunning ✨", fromMe: true, time: "10:00 AM" },
    { id: 2, text: "Thank you so much! I put so much love into every shot 📸", fromMe: false, time: "10:04 AM" },
    { id: 3, text: "New content coming soon 🌙", fromMe: false, time: "10:07 AM" },
  ],
  4: [
    { id: 1, text: "I love your work!", fromMe: true, time: "3:18 PM" },
    { id: 2, text: "omg that is so sweet!! 😍 You're the best", fromMe: false, time: "3:20 PM" },
    { id: 3, text: "Loved your comment!", fromMe: false, time: "3:22 PM" },
  ],
  5: [
    { id: 1, text: "The beach content has been incredible lately 🌊", fromMe: true, time: "9:00 AM" },
    { id: 2, text: "Thank you! The ocean always inspires me 💙", fromMe: false, time: "9:04 AM" },
    { id: 3, text: "Check out my latest post", fromMe: false, time: "9:10 AM" },
  ],
  6: [
    { id: 1, text: "That last set you posted was fire 🔥", fromMe: false, time: "8:25 AM" },
    { id: 2, text: "Thank you!! Working on something even better 💜", fromMe: true, time: "8:28 AM" },
...
export default function ChatSidebar({ contacts = [] }) {
  ...
  useEffect(() => {
    try {
      if (activeChatId != null) localStorage.setItem(STORAGE_KEY, String(activeChatId));
    } catch {}
  }, [activeChatId]);
  ...
  const openChat = useCallback(
    (preferredId) => {
      clearTimeout(closeTimerRef.current);
      const valid =
        contacts.find((c) => c.id === preferredId)?.id ?? contacts[0]?.id ?? null;
      setActiveChatId(valid);
      if (valid != null) {
        setReadIds((prev) => (prev.has(valid) ? prev : new Set([...prev, valid])));
      }
      setChatMounted(true);
      if (reducedMotion.current) {
        setChatVisible(true);
  ...
  const activeContact = localContacts.find((c) => c.id === activeChatId) ?? null;
          ...
          >
            ...
            <div className="w-[272px] shrink-0 flex flex-col border-r border-pink-100">
              ...
              <div className="flex items-center justify-between px-4 py-3 border-b border-pink-50">
                <h2 className="text-sm font-semibold text-[#241a22]">Messages</h2>
                <button
                  onClick={closeChat}
              ...
              {/* Tabs */}
              <div className="flex border-b border-pink-50" role="tablist" aria-label="Message filters">
                {(["all", "unread"]).map((tab) => (
                  <button
                    key={tab}
                    role="tab"
                    ...
                    className={`flex-1 py-2 text-xs font-medium text-center transition cursor-pointer relative ${
                      activeTab === tab ? "text-[#241a22]" : "text-[#b89aa8] hover:text-[#8c6d7f]"
                    }`}
                  >
                    {tab === "all" ? "All" : "Unread"}
                    {activeTab === tab && (
              ...
              {/* Contact list */}
              <div className="flex-1 overflow-y-auto hide-scrollbar" role="tabpanel">
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center justify-between gap-2">
                          <p className={`text-sm truncate ${contact.unread > 0 ? "font-semibold text-[#241a22]" : "font-medium text-[#241a22]"}`}>
                            {contact.name}
                          </p>
                          <span className="text-[10px] text-[#b89aa8] shrink-0">{contact.time}</span>
                        </div>
                        <p className="mt-0.5 text-xs text-[#8c6d7f] truncate">
                          {contact.fromMe ? `You: ${contact.lastMessage}` : contact.lastMessage}
                        </p>
                      </div>
                    </button>
                  ))
            ...
            <div className="flex-1 flex flex-col min-w-0">
              {activeContact ? (
                <>
                  ...
                  <div className="flex items-center gap-3 px-5 py-3 border-b border-pink-100 bg-white shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-semibold text-[#241a22] truncate">{activeContact.name}</p>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <MenuHeartIcon status={activeContact.status} size={12} />
                        <span className="text-[11px] text-[#8c6d7f]">
                          {STATUS_OPTIONS.find((s) => s.id === activeContact.status)?.label ?? "Online"}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex-1 overflow-y-auto hide-scrollbar px-5 py-5 space-y-4 bg-[#fffafc]">
                    {allMessages.map((msg) => (
                      ...
                      >
                        ...
                        <div className={`flex flex-col gap-0.5 ${msg.fromMe ? "items-end" : "items-start"}`}>
                          <div
                          ...
                          <span className="text-[10px] text-[#c4a8b8] px-1">{msg.time}</span>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="px-4 pt-2.5 pb-3 border-t border-pink-100 bg-white shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 mb-2">
                      ...
                      <button
                        ...
                        style={{ background: "linear-gradient(90deg,#f5b63b,#f9c76a)", color: "#2B1A10" }}
                ...
                <div className="flex-1 flex flex-col items-center justify-center gap-3 text-[#b89aa8] select-none">
                  <svg viewBox="0 0 48 48" className="w-12 h-12 opacity-25" fill="none" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round">
      ...
      {chatTooltip && !chatMounted && (
        ...
        >
          <div className="bg-white border border-pink-100 rounded-xl shadow-lg px-3 py-2 w-44">
            <p className="text-xs font-semibold text-[#241a22] truncate">{chatTooltip.name}</p>
            <p className="mt-0.5 text-[11px] text-[#8c6d7f] truncate">
              {chatTooltip.fromMe ? `You: ${chatTooltip.lastMessage}` : chatTooltip.lastMessage}
            </p>
            <div className="flex items-center gap-1.5 mt-1.5 pt-1.5 border-t border-pink-50">
              ...
              <span className="text-[11px] text-[#8c6d7f]">
                {STATUS_OPTIONS.find((s) => s.id === chatTooltip.status)?.label ?? "Online"}
              </span>
            </div>
          </div>
        </div>
