...
export default function HomePage({ userStatus = 'online', onStatusChange }) {
  ...
  return (
    <MemberLayout
      ...
      onLogoClick={() => { setFeedTab("foryou"); window.scrollTo({ top: 0, behavior: "smooth" }); }}
      ...
      `}</style>
        ...
        <main className="flex-1 min-w-0 pb-20 md:pb-8">
          {page.status === "loading" ? (
            ...
            <ErrorState message="We couldn’t load your feed." onRetry={page.retry} />
          ) : page.status === "empty" ? (
            <EmptyState title="No posts yet" message="Follow some creators to see their posts here." />
          ) : (
            <>
          ...
          >
            ...
            >
              ...
              <div className="px-6 pt-5 pb-4">
                ...
                {/* Upload Disclaimer */}
                <label className={`mt-2 flex cursor-pointer items-start gap-2.5 rounded-xl border px-3 py-2.5 transition ${composeRightsConfirmed ? "border-pink-200 bg-pink-50/40" : "border-pink-100 bg-white/30"}`}>
                  <input
                    type="checkbox"
                    checked={composeRightsConfirmed}
                    onChange={(e) => setComposeRightsConfirmed(e.target.checked)}
                  ...
                  <span className="text-[10px] leading-relaxed text-[#9e8090]">
                    ...
                    <strong className="text-[#8c6d7f]">Acceptable Use</strong> and{" "}
                    <strong className="text-[#8c6d7f]">Non-Consensual Content Policies</strong>.
                  </span>
                </label>
                ...
                <div className="mt-2 flex justify-end">
                  <button
                    ...
                    onClick={() => { setShowCompose(false); setComposeLocked(false); setComposeVesoPrice(""); setComposeText(""); setComposeRightsConfirmed(false); }}
