  ...
  const filtered = useMemo(() => {
    return transactions.filter((t) => {
      ...
      if (
        ...
        !t.who.toLowerCase().includes(q.toLowerCase())
      )
        return false;
      return true;
    });
            ...
            <tbody className="divide-y divide-pink-50">
              {filtered.map((t) => (
                <tr key={t.id} className="hover:bg-pink-50/30">
                  <td className="whitespace-nowrap px-5 py-3.5 text-[#8c6d7f]">
                    {t.when}
                  </td>
                  ...
                  <td
                    className={`whitespace-nowrap px-5 py-3.5 text-right font-bold tabular-nums ${t.direction === "in" ? "text-emerald-600" : "text-[#241a22]"}`}
                  >
                    {t.direction === "in" ? "+" : "−"}
                    {money(t.amount)}
                  </td>
  ...
  return (
    <>
      <div className="mb-5">
        ...
        <p className="mt-1 text-sm text-[#8c6d7f]">
          Manage your active subscriptions, toggle auto-renew, and view
          spending.
        </p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3 mb-5">
        ...
        <StatCard
          label="Monthly spend"
          value={money(activeSubscriptions.reduce((s, x) => s + x.price, 0))}
          icon={
            <svg
              viewBox="0 0 24 24"
              className="w-5 h-5"
          ...
          {activeSubscriptions.map((sub) => (
            <li key={sub.id} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-0.5 text-xs text-[#8c6d7f]">
                  Renews {sub.renews} · {money(sub.price)}/mo
                </div>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                ...
                <div className="flex flex-col items-center gap-1">
                  <Toggle
                    ...
                    onChange={() => {}}
                    label={`Auto-renew ${sub.creator}`}
                  />
                  <span className="text-[10px] font-semibold text-[#8c6d7f]">
      ...
      <div className="mt-4 rounded-2xl border border-pink-100 bg-gradient-to-br from-pink-50 via-white to-white p-5 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#5b4153]">
          Set a monthly cap so subscriptions don't exceed your budget.
        </p>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full rounded-xl border border-pink-100 bg-[#fffafc] px-3 py-2.5 text-sm outline-none placeholder:text-[#c59aae] focus:border-pink-300"
              />
            </div>
            <div>
              <label className="mb-1 block text-[11px] font-semibold uppercase tracking-wider text-[#b89aa8]">
              ...
              <div className="flex gap-2">
                {[5, 10, 25, 50, 100].map((v) => (
                  <button
                    key={v}
                    className="flex-1 rounded-xl bg-pink-50 py-2.5 text-sm font-bold text-[#5b4153] transition hover:bg-[#df5f97] hover:text-white"
                  >
                    ${v}
                  </button>
                ))}
              ...
              <div className="mt-2 flex items-center rounded-xl border border-pink-100 bg-[#fffafc]">
                ...
                <input
                  defaultValue="25.00"
                  className="w-full bg-transparent py-2.5 pr-3 text-sm font-bold outline-none tabular-nums"
                />
              </div>
            </div>
            <div>
              ...
              <li key={l.id} className="flex items-center gap-3 px-5 py-3.5">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5">
                    ...
                    <Pill tone={l.direction === "received" ? "green" : "pink"}>
                      {l.direction === "received" ? "Received" : "Sent"}
                    </Pill>
                  </div>
                  <div className="text-xs text-[#8c6d7f]">{l.when}</div>
                </div>
                <span
                  className={`text-sm font-bold tabular-nums ${l.direction === "received" ? "text-emerald-600" : "text-[#241a22]"}`}
                >
    ...
    <>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {paymentMethods.map((pm) => (
          <div
            key={pm.id}
            className={`rounded-2xl border bg-white p-5 shadow-sm ${pm.isDefault ? "border-[#df5f97] ring-1 ring-[#df5f97]" : "border-pink-100"}`}
          >
            <div className="flex items-start justify-between">
              <div
                className={`flex h-10 w-16 items-center justify-center rounded-lg text-sm font-bold ${pm.brand === "visa" ? "bg-blue-50 text-blue-600" : pm.brand === "mc" ? "bg-orange-50 text-orange-600" : "bg-sky-50 text-sky-600"}`}
              >
            ...
            <div className="mt-3">
              <div className="text-lg font-bold text-[#241a22] tracking-widest">
                {pm.type === "PayPal" ? pm.last4 : `···· ···· ···· ${pm.last4}`}
              </div>
            ...
            <div className="mt-4 flex gap-2">
              ...
              <button className="flex-1 rounded-xl border border-pink-200 bg-white py-2 text-xs font-semibold text-[#5b4153] transition hover:bg-pink-50">
                Edit
              </button>
              <button className="rounded-xl border border-red-100 bg-white px-3 py-2 text-xs font-semibold text-red-500 transition hover:bg-red-50">
                Remove
              </button>
            </div>
          </div>
          ...
          <div className="space-y-2">
            {[
              ...
              >
                ...
                {d.ready ? (
                  <button className="rounded-lg px-3 py-1 text-xs font-semibold text-[#df5f97] ring-1 ring-pink-200 hover:bg-pink-50">
                    Download PDF
                  </button>
                ) : (
