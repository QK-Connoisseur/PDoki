...
export default function LoginPage({ onLogin }) {
  ...
  const footerGroups = [
    {
      title: "Platform",
      links: ["Explore Pumdoki", "Become a Creator", "About", "Contact"],
    },
    {
      title: "Support",
      links: ["Help", "Support", "Cookie Notice"],
    },
    {
      title: "Legal",
      links: ["Terms of Service", "Privacy", "DMCA", "USC 2257"],
    },
  ];
  ...
  const socialLinks = [
    ...
    },
    {
      ...
      icon: (
        <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current" aria-hidden="true">
          <path d="M14 2h3.14c.19 1.593 1.087 3.028 2.45 3.885.892.56 1.93.864 2.41.925v3.22a8.11 8.11 0 0 1-4-.987v6.264A6.307 6.307 0 0 1 11.693 22 6.307 6.307 0 0 1 5.386 15.693a6.307 6.307 0 0 1 6.307-6.307c.344 0 .686.028 1.02.083v3.277a3.18 3.18 0 0 0-1.02-.167 3.114 3.114 0 0 0 0 6.228 3.114 3.114 0 0 0 3.114-3.114V2Z" />
        </svg>
      ),
    },
    {
      ...
      icon: (
        <svg viewBox="0 0 24 24" className="h-5 w-5 fill-current" aria-hidden="true">
          <path d="M20.317 4.369A19.791 19.791 0 0 0 15.419 3c-.211.375-.458.877-.627 1.276a18.27 18.27 0 0 0-5.584 0A13.04 13.04 0 0 0 8.58 3a19.736 19.736 0 0 0-4.899 1.369C.533 9.012-.32 13.54.107 18.003A19.93 19.93 0 0 0 6.13 21c.487-.667.923-1.374 1.297-2.114-.713-.27-1.39-.6-2.03-.983.17-.124.338-.252.499-.383 3.92 1.84 8.174 1.84 12.047 0 .164.134.332.262.499.383-.64.386-1.321.716-2.034.986.375.737.811 1.444 1.297 2.111a19.873 19.873 0 0 0 6.027-2.997c.501-5.173-.856-9.659-3.415-13.634ZM8.019 15.331c-1.182 0-2.15-1.084-2.15-2.418 0-1.333.95-2.417 2.15-2.417 1.209 0 2.168 1.094 2.15 2.417 0 1.334-.95 2.418-2.15 2.418Zm7.962 0c-1.182 0-2.15-1.084-2.15-2.418 0-1.333.95-2.417 2.15-2.417 1.209 0 2.168 1.094 2.15 2.417 0 1.334-.941 2.418-2.15 2.418Z" />
        </svg>
      ),
    },
  ];
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#fff8fb] text-[#5b4153]">
          ...
          <div className="relative z-10 mx-auto w-full max-w-md">
            <div className="rounded-[32px] border border-pink-100 bg-white p-6 shadow-[0_20px_60px_rgba(244,114,182,0.15)] md:p-8">
              ...
              <div className="mt-6 rounded-2xl bg-pink-50 p-4 text-sm leading-6 text-[#8a6a7c]">
                ...
                <button className="mt-3 font-semibold text-[#df5f97] hover:underline">Create account</button>
              </div>
            </div>
          </div>
        </section>
      ...
      <footer className="relative z-10 border-t border-pink-100 bg-white/85 px-6 py-8 backdrop-blur-sm md:px-10 lg:px-14 xl:px-20">
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-8 md:grid-cols-3">
            {footerGroups.map((group) => (
              <div key={group.title}>
                <h3 className="text-sm font-semibold uppercase tracking-[0.18em] text-[#8c6d7f]">
                  {group.title}
                </h3>
                <div className="mt-4 space-y-3 text-sm text-[#7f6274]">
                  {group.links.map((item) => (
                    <a key={item} href="#" className="block transition hover:text-[#df5f97]">
                      {item}
                    </a>
                  ))}
                </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <p className="text-sm text-[#9e8090]">&copy; 2026 Pumdoki. Built for presence, interaction, and connection.</p>
            <div className="flex items-center gap-3 text-[#9e8090]">
              {socialLinks.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  aria-label={item.name}
                  className="rounded-full border border-pink-100 bg-white p-2 transition hover:border-pink-200 hover:text-[#df5f97]"
                >
                  {item.icon}
                </a>
              ))}
