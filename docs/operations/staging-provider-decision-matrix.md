...
The founder-approved provider-neutral architecture direction is recorded in
[`../architecture/phase2-async-work-throttling-idempotency.md`](../architecture/phase2-async-work-throttling-idempotency.md).
It locks PostgreSQL durability, ephemeral-only Redis use, at-least-once worker
semantics, and conservative outage behavior. It does not fill the provider
scorecards below or authorize evaluation, provisioning, spending, staging, or
production use.

| Responsibility                                | Required source of truth                                                                                                         | Permitted Redis/cache role                                           | Required outage decision                                                                                                                                                   |
| --------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Login/current-password/reset/email throttling | Security policy plus auditable application configuration                                                                         | Shared counters and bounded windows across API instances             | Follow the ADR's route policy: stricter bounded local guard for login/current password; verification `503`; reset stays neutral `202` with no side effect; never unlimited |
| Registration throttling                       | Security policy plus PostgreSQL account constraints                                                                              | Shared per-client request counter                                    | Use a strict bounded local allowance; a tracked threshold returns `429`, while an unavailable/full fallback returns `503`; never leave account creation unlimited          |
| General API rate limiting                     | Application/edge policy                                                                                                          | Shared counters or coordination                                      | Use bounded local enforcement only with a hard replica ceiling and edge limit; otherwise fail closed. Exact edge ownership remains undecided                               |
| Background jobs                               | PostgreSQL job/outbox intent committed with the applicable domain mutation                                                       | Optional wake-up/coordination only                                   | Required intent and domain state commit together or neither does; worker outage preserves backlog with bounded retry/dead handling and authorized replay                   |
| Email delivery jobs                           | PostgreSQL delivery intent if asynchronous sending is introduced                                                                 | Optional wake-up and retry scheduling                                | Provider/cache outage cannot falsely report durable acceptance or recipient delivery; ambiguous at-least-once delivery may duplicate and must be tolerated                 |
| Payment webhook idempotency                   | PostgreSQL record with provider/internal-integration/event uniqueness, transaction state, and ledger effects in an atomic design | Optional short-lived duplicate suppression or lock optimization only | Payment processing remains safe with Redis unavailable; cache loss may increase DB work, never duplicate a ledger effect                                                   |
...
| Session revocation                            | PostgreSQL `Session` lifecycle in the current design                                                                             | None approved by the current ADR                                     | Redis is not in the session authority/read path; revoked sessions remain revoked                                                                                           |
...
The Phase 2 ADR already selects PostgreSQL as the durable authority and limits
Redis to shared throttling and ephemeral coordination. In the rows below,
`TBD` or `UNDECIDED` applies only to the implementation, provider, topology,
and provisional/final operational acceptance; it does not reopen that approved
...
| Redis/cache         | `Ephemeral role locked; provider TBD`              | `[TBD]`               | `[OUTSIDE GIT]`                 | `[OUTSIDE GIT]`       | `[OUTSIDE GIT]`  | Multi-instance rollout or security-policy change    |
| Queue               | `PostgreSQL durability locked; implementation TBD` | `[TBD]`               | `[OUTSIDE GIT]`                 | `[OUTSIDE GIT]`       | `[OUTSIDE GIT]`  | First asynchronous consequential workflow           |
| Durable idempotency | `PostgreSQL locked; operation design TBD`          | `[TBD]`               | `[OUTSIDE GIT]`                 | `[OUTSIDE GIT]`       | `[OUTSIDE GIT]`  | Before any payment or ledger integration            |
