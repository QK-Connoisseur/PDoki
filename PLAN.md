...
- The separately authorized local
  ...
  execute an isolated migration fixture or worker retry/shutdown lifecycle.
  Candidate selection remains open pending completion of those checks.
- The separately authorized
  ...
  worker, provider, schema, service, deployment, or live configuration.
...
| Async-foundation implementation/provider review  | Engineering + Founder | 2026-08-25 | Transaction/privilege sub-proof and local Node 24 baseline verification passed; no worker/library was installed. Open and review the spike and runtime draft PRs, require separate merge approval, then finish current-candidate exact least-privilege, isolated-migration, stale-attempt, retry, and shutdown evaluation before selecting the local foundation. |
...
7. PR #8 published the provider-neutral Phase 2 async-work ADR. Open and review
   the local transaction/privilege and stacked Node 24 baseline draft PRs. If
   their merges are separately approved, merge the spike first, retarget the
   runtime PR to `dev`, and reverify its isolated diff and CI. Afterward, finish candidate exact
