...
Pumdoki is an adult creator platform (subscriptions, paid content, creator services/bookings, real-time messaging, prepaid "Veso" credits, and a collectible "Oasis/Drimy" retention game). The repo is an npm-workspaces monorepo: a **React frontend prototype** in `apps/web`, an independently buildable private operations shell in `apps/admin`, a TypeScript Express API in `apps/api`, shared Zod schemas in `packages/contracts`, and Prisma + PostgreSQL in `packages/database`. Phase 3 slices 1–3 are published and CI-verified: core auth, email verification/password reset, and frontend auth integration. Slice 4 is in progress with a locally verified persisted explicit-content preference and protected Settings page. Broader account Settings, payments, media, operational workflows, and other product APIs remain incomplete. `packages/ui` is still empty scaffolding.
...
- `docs/architecture/` — durable slice designs, implementation records, and architecture decisions. Slice 3 and the in-progress Slice 4 preference design are recorded there.
...
The monorepo migration is committed: work against `apps/web` and `apps/api`, never old top-level paths. Phase 2 is **partially complete (local foundation)** — staging deploy, RDS, backups, Sentry, job queue, and idempotency remain deferred. Phase 3 slices 1–3 (core auth backend, email flows, and frontend integration) are published and CI-verified. Slice 4 is in progress: the explicit-content preference vertical cut is locally implemented and verified, while broader account Settings and future server-side content-query enforcement remain. See `HANDOFF.md` for exact commands, counts, and publication status.
