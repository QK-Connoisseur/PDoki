...
Pumdoki is an adult creator platform (subscriptions, paid content, creator services/bookings, real-time messaging, prepaid "Veso" credits, and a collectible "Oasis/Drimy" retention game). The repo is an npm-workspaces monorepo: a **React frontend prototype** in `apps/web`, an independently buildable private operations shell in `apps/admin`, a TypeScript Express API in `apps/api`, shared Zod schemas in `packages/contracts`, and Prisma + PostgreSQL in `packages/database`. Phase 3 slices 1–3 are published and CI-verified. Slice 4A is committed locally but unpushed; Slice 4B is implemented and fully locally verified in the working tree. Together they complete the founder-approved Phase 3 core scope: persisted explicit-content preference plus profile, email, password, and active-session Settings. Manual review, commit/publish, and green CI remain before Phase 4 implementation begins. Dependency-bound Settings, payments, media, operational workflows, and other product APIs remain incomplete. `packages/ui` is still empty scaffolding.
...
- `docs/architecture/` — durable slice designs, implementation records, and architecture decisions, including the Slice 4A preference and Slice 4B account-security designs.
...
The monorepo migration is committed: work against `apps/web` and `apps/api`, never old top-level paths. Phase 2 is **partially complete (local foundation)** — staging deploy, RDS, backups, Sentry, job queue, and idempotency remain deferred. Phase 3 slices 1–3 are published and CI-verified. Slice 4A is the local unpushed commit `7972bf2`; Slice 4B is fully locally verified but uncommitted. Their founder-approved core scope is ready for manual review and publication. Notifications, theme, billing, export/deletion, and server-side content-query enforcement remain deliberately sequenced to their dependency phases. See `HANDOFF.md` for exact commands, counts, and publication status.
